...
import type { FhevmInitSDKOptions } from "../fhevmTypes";
...
let __relayerInitialized = false;
const fhevmInitSDK = async (options?: FhevmInitSDKOptions) => {
  const relayerSDK = await loadRelayerSDK();
  if (__relayerInitialized) return true;
  // 显式传入 wasm 资源路径，避免在不同打包/部署环境下推断失败导致初始化卡住
  const result = await relayerSDK.initSDK({
    tfheWasmUrl: "/tfhe_bg.wasm",
    kmsWasmUrl: "/kms_lib_bg.wasm",
    ...(options as Record<string, unknown> | undefined),
  });
  if (!result) throw new Error("relayerSDK.initSDK failed.");
  __relayerInitialized = true;
  return true;
};

function checkIsAddress(a: unknown): a is `0x${string}` {
  if (typeof a !== "string") return false; if (!isAddress(a)) return false; return true;
}
...
async function tryFetchFHEVMHardhatNodeRelayerMetadata(rpcUrl: string): Promise< { ACLAddress: `0x${string}`; InputVerifierAddress: `0x${string}`; KMSVerifierAddress: `0x${string}`; } | undefined > {
  ...
  try {
    ...
    if (!("ACLAddress" in metadata && typeof (metadata as any).ACLAddress === "string" && (metadata as any).ACLAddress.startsWith("0x"))) return undefined;
    if (!("InputVerifierAddress" in metadata && typeof (metadata as any).InputVerifierAddress === "string" && (metadata as any).InputVerifierAddress.startsWith("0x"))) return undefined;
    if (!("KMSVerifierAddress" in metadata && typeof (metadata as any).KMSVerifierAddress === "string" && (metadata as any).KMSVerifierAddress.startsWith("0x"))) return undefined;
...
async function getFHEVMRelayerMetadata(rpcUrl: string) {
  const rpc = new JsonRpcProvider(rpcUrl);
  try { const version = await rpc.send("fhevm_relayer_metadata", []); return version; }
  catch (e) { throwFhevmError("FHEVM_RELAYER_METADATA_ERROR", `The URL ${rpcUrl} is not a FHEVM Hardhat node or is not reachable. Please check the endpoint.`, e); }
  finally { rpc.destroy(); }
}
...
export const createFhevmInstance = async (parameters: {
  ...
  if (isMock) {
    ...
    if (fhevmRelayerMetadata) {
      ...
      const mockInstance = await fhevmMock.fhevmMockCreateInstance({ rpcUrl, chainId, metadata: fhevmRelayerMetadata });
      throwIfAborted();
      return mockInstance;
    }
  }
